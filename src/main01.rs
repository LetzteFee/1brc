...
impl Station {
    ...
    fn drain(self, station_name: &str) {
        ...
        print!("{}={:.1}/{:.1}/{:.1}", station_name, self.min, mean, self.max);
    }
}
...
fn process(mut file: fs::File, map: &mut HashMap<String, Station>) {
    ...
    loop {
        ...
        match str::from_utf8(&buffer) {
            Ok(k_string) => string = k_string,
            Err(u8r) => {
                valid_len = Some(u8r.valid_up_to());
                string = str::from_utf8(&buffer[..u8r.valid_up_to()]).unwrap()
            }
        }
        ...
        // process data
        for line in lines {
            if process_line(line, map).is_none() {
                println!("{}", line);
                panic!("");
            }
        }
        ...
        buffer_offset = 0;
        for (index, byte) in last_line.as_bytes().into_iter().enumerate() {
            buffer[index] = *byte;
...
fn process_line(line: &str, map: &mut HashMap<String, Station>) -> Option<()>{
    let mut line_iter = line.split(";");
    let name: &str = line_iter.next()?;
    let value_str: &str = line_iter.next()?;
